...
export function parseLocationSlug(slug) {
  ...
  const cityName = parts.slice(0, -1).map(capitalize).join(" ");

  // Parse state (the last part)
  const stateName = capitalize(parts[parts.length - 1]);
  ...
  return { city: cityName, state: stateName, country: "India", isValid: true };
}
