...
export default function EditEventForm({ event }) {
    ...
    const { mutate: updateEvent, isLoading } = useConvexMutation(
    ...
    const inputClasses = cn("text-white placeholder:text-white/40 transition-colors border bg-[#0A0A0A] border-[#27272A] focus-visible:ring-primary/50");
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const updatedEventId = await updateEvent({
                eventId: event._id,
                title: data.title,
            ...
            if (data.title !== event.title) {
                // If title changed, the slug changed, we need to redirect to new slug
                // We'd need the new slug, but we don't return it currently. Let's just go to my-events.
