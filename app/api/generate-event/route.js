...
export async function POST(req) {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return NextResponse.json(
        { error: "Gemini API key is not configured on the server. Please restart your dev server if you just added it to .env.local" },
        { status: 500 }
      );
    }
...
`;
    ...
    if (error?.status === 429 || error?.message?.includes("429")) {
      return NextResponse.json(
        { error: "AI service quota exceeded. Please try again later or check your Gemini API billing." },
        { status: 429 }
      );
    }
